...
public class TC015_OFOS_MyOrder extends Base_Test{

	@Test
	public void placeOrderFromAmericanRestaurant() throws InterruptedException {
	
		TC002_OFOS_Login login = new TC002_OFOS_Login();
		login.login();
		
		Welcome_Page wp = new Welcome_Page(driver);
		js.executeScript("arguments[0].scrollIntoView(true)", wp.getAmericanLink());
		Thread.sleep(2000);
		clickAction(wp.getAmericanLink());
		Thread.sleep(2000);
		
		for (int i = 0; i < wp.getAllRestaurants().size(); i++) {
			if (wp.getAllRestaurants().get(i).getText().equals("Highlands Bar & Grill")) {
